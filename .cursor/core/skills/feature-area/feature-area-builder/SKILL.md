...
Before executing `map`, `validate`, `promote`, `slice`, `scaffold-slices`, or a **cold-start** `scaffold` (no in-thread approved map from `map`), confirm that a Feature Area Context Brief has been produced by the Feature Area Lead (`.cursor/core/agents/feature-area/feature-area-lead.md`) for this command flow.
...
4. Cross-check every proposed Feature Area against the v0 exclusion list in `.cursor/core/rules/feature-area-workflow.mdc` §6. If a proposed area is entirely deferred, mark it `deferred` and exclude from the active list.
5. Check existing `docs/product/feature-areas/` files for overlap or gaps.
6. Invoke Scope Critic (`.cursor/core/agents/feature-area/scope-critic.md`) to review the proposal before presenting it to the user.
7. Present the map proposal using the output format in `.cursor/core/commands/feature-area.md` Mode: map.
...
- Feature Area files — use `/feature-area scaffold` after approval (writes from `.cursor/core/templates/product/feature-area.template.md`)
- Scope Slices (those come after validation)
- Architecture diagrams or service boundaries

## 5. Mode: scaffold

Write initial Feature Area files after the user **approves** a Feature Area Map in-context. Governed by `.cursor/core/commands/feature-area.md` Mode: scaffold.
...
3. If missing or **empty-only**, instantiate from **`.cursor/core/templates/product/feature-area.template.md`** (keep template structure and headings).
...
Use the result format in `.cursor/core/commands/feature-area.md` (Created / Skipped / NEED_HUMAN list / next `/feature-area validate <kebab-name>`, then `/feature-area promote <kebab-name>` after CLEAR).

## 6. Mode: validate

Run FA-01–FA-09 and CC-02–CC-05 from `.cursor/core/checkers/scope-readiness-checker.md` against a Feature Area file.
...
5. Output the summary table from `.cursor/core/checkers/scope-readiness-checker.md` Summary Output Format.
...
Governed by `.cursor/core/commands/feature-area.md` Mode: promote.
...
5. **Only if CLEAR**, apply **only** these edits to the Feature Area file:
   ...
   - `## Changelog` → append one row; use the exact pipe-row text specified in `.cursor/core/commands/feature-area.md` Mode: promote (`YYYY-MM-DD` = current calendar date).

No other sections or files.

### Output

Use the result format in `.cursor/core/commands/feature-area.md` (including no-op and BLOCKED cases).
...
1. Read `docs/product/feature-areas/<feature-area-name>.md`.
2. `Status` must be `validated`. If not: stop and output the gate message from `.cursor/core/commands/feature-area.md` Mode: slice.
...
6. Invoke Scope Critic (`.cursor/core/agents/feature-area/scope-critic.md`) to review the proposal.
7. Present using the output format in `.cursor/core/commands/feature-area.md` Mode: slice.
...
- Scope Slice files — use `/feature-area scaffold-slices <name>` after explicit approval (writes from `.cursor/core/templates/product/scope-slice.template.md`)
- User stories, specs, or tasks
- Data models, API routes, or technology choices

## 9. Mode: scaffold-slices

Materialize an **approved** Scope Slice proposal into files. Governed by `.cursor/core/commands/feature-area.md` Mode: scaffold-slices.
...
3. If missing or empty-only: instantiate from **`.cursor/core/templates/product/scope-slice.template.md`**.
4. **Fill only** the product-level sections listed in `.cursor/core/commands/feature-area.md` Mode: scaffold-slices (Parent Feature Area, Status, flags, User Value, Exact Boundary, credit/sharing/feedback impacts, Dependencies, Blockers, Acceptance-Level Outcome, Changelog). Default **Status** to `exploratory` unless the proposal row was `blocked` or `deferred`.
5. Leave **UX States**, **Data Touched**, and **Readiness for User Stories** (and embedded verdict) for **`refine-slice`** / **`promote-slice`** per `.cursor/core/commands/feature-area.md` (scaffold leaves UX States and Data Touched empty; Readiness is promotion-only).
...
Use the result format in `.cursor/core/commands/feature-area.md` (Created / Skipped / next `/feature-area refine-slice` per file).
...
Edit **product-level** sections of **one** Scope Slice file. Governed by `.cursor/core/commands/feature-area.md` Mode: refine-slice.
...
4. Ground in parent Feature Area, PRD, open questions; **PRD-allowed product-level terms** per `.cursor/core/checkers/scope-readiness-checker.md`.
5. **Do not** write user stories, specs, tasks, architecture, services, APIs, or implementation detail.

### Output

Use the result format in `.cursor/core/commands/feature-area.md`.

## 10.2 Mode: promote-slice

Apply the narrow **ready-for-user-stories** transition after **CLEAR** (SS-01–SS-10, CC-01–CC-05). Governed by `.cursor/core/commands/feature-area.md` Mode: promote-slice.
...
5. **Only if CLEAR**, apply **only** the four edits defined in `.cursor/core/commands/feature-area.md` Mode: promote-slice (Status, Readiness checklist all `[x]`, Verdict line, Changelog row).
