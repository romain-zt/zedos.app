...
export const userStoryCorpora = pgTable(
  ...
  (t) => [
    unique('user_story_corpora_project_cluster_unique').on(t.projectId, t.featureSplitClusterId),
    index('user_story_corpora_project_id_idx').on(t.projectId),
    index('user_story_corpora_cluster_id_idx').on(t.featureSplitClusterId),
  ]
);
...
export const userStoryLines = pgTable(
  ...
  (t) => [
    unique('user_story_lines_corpus_sort_unique').on(t.corpusId, t.sortOrder),
