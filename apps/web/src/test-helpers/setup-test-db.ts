...
import { createTestDb, sql } from '@repo/db';
...
const testConnection = createTestDb(TEST_DATABASE_URL);
...
export async function truncateCreditTables(): Promise<void> {
  await testConnection.db.execute(
    sql`TRUNCATE TABLE credit_transactions, processed_webhook_events, purchases RESTART IDENTITY CASCADE`,
  );
}
