...
On every `/prd challenge` invocation, regardless of user prompt, you must run all eight checks defined in `.cursor/core/commands/prd.md` § "Default challenge scope":
...
Every `/prd challenge` response must use the **Challenge Report** format defined in `.cursor/core/commands/prd.md` § "Required output format". No free-form challenge summaries. No partial sections.
