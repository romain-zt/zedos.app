...
const logger = createLogger({ operation: 'account/export' });
...
export async function GET() {
  ...
  try {
    ...
    if (!parseResult.success) {
      logger
        .withContext({ userId })
        .error('Account export outbound validation failed', validationFailureData(parseResult.error.flatten()));
    ...
    logger.withContext({ userId }).error('Account export GET failed', error);
